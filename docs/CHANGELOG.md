...
- **捕获开关**:托盘右键「启用捕获」勾选 + 快捷键设置对话框顶部状态条(方案 C:● 捕获中·双击 Ctrl+C 翻译 + iOS 开关),两处联动同步(共用 `config.capture_enabled`,状态条在「完成」时写入、取消则丢弃)。关闭时复制触发与唤起键都不弹窗,托盘手动调出仍可用;关闭时托盘图标变灰、tooltip「捕获已暂停」。`ADR-0004`。
- **复制触发键**(默认 Ctrl+C):捕获从"任意复制即触发"改为"按下复制触发键才触发"——默认 Ctrl+C 需 500ms 内连按两次(双击),自定义组合键单击即触发;第二次按下时剪贴板没刷新则不触发(避免双击没选中文字时误翻旧内容)。
- **可配置复制触发键**:快捷键设置窗新增「复制触发键」行(默认 `ctrl+c`),录制式修改;须含修饰键、不能与唤起/关闭相同;默认值显示「双击触发」、自定义显示「单击触发」徽标。config 新增 `copy_hotkey`。

### 关键改动 / 边界

| 问题 | 根因 | 解决 |
|---|---|---|
| 双击 Ctrl+C 复制相同文本被去重压掉 | 原 `_emit_if_new` 有 2s 去重 | 双击/单击是显式"翻译这段"手势,触发路径绕过去重直接 emit |
| 托盘勾选与 ⚙ 开关不同步 | 托盘菜单一次性构建,勾选态只在构建时设置 | `_update_tray_state` 在状态变化时 `setChecked` 同步 |
| 双击 Ctrl+C 在终端里是中断信号 | 终端 Ctrl+C 不写剪贴板 | 剪贴板刷新门控:两次按下之间 `dataChanged` 未触发则不弹窗 |
| **Qt 事件循环运行期间重建 pynput 监听器死锁** | `keyboard.Listener`/`GlobalHotKeys` 构造时 `KeyTranslator` 调 `MapVirtualKeyEx`,主线程在 Qt 消息循环中即卡死(>30s 不复返);设置里改热键必经此路径 | 改单一常驻 `keyboard.Listener` + 纯 Python `pynput.HotKey` 做组合匹配;启动时(事件循环未跑)主线程同步构造,改热键只换 HotKey 对象、不重建监听器 |

---

## 落地页 · GitHub Pages 推广(grilling-with-docs 会话)

通过「grilling + domain-modeling + prototype」会话敲定推广定位并上线:

- **定位纠偏**:主卖点不是"免切换",而是**不是又一个翻译软件**——翻译 + 解释合进同一个悬浮窗,均由 DeepSeek 大模型驱动;主打读论文时术语 / 名词的物理意义不清楚、需要当场讲清的场景。
- **对外用词**:新增「用户向用语」映射(悬浮窗 / 双击 Ctrl+C / 历史记录 / 一键暂停捕获…),内部术语与对外文案分层不打架,见 [`CONTEXT.md`](CONTEXT.md)。
- **产物**:单文件落地页 `index.html`(浅色清爽风)+ 6 张真实截图(`img/`),GitHub Pages 上线 **<https://reamuroray.github.io/DSTranslator/>**;README 顶部挂主页链接。
- **真实截图**:由用户手动拍摄(翻译 / 解释 / 场景 / 历史 / 设置 / 托盘),嵌入页面功能区。

### 关键决策

| 决策 | 选择 |
|---|---|
| 页面形态 | 单页转化漏斗(hero → 为什么 → 交互演示 → 功能 → 三分钟上手 → 下载 → FAQ),不用多页文档站 |
| 呈现方式 | hero 用 CSS 高仿窗做交互演示,功能区用真实截图(混合),规避当时仓库零截图的问题 |
| 托管 | GitHub Pages(main / root),单自包含 HTML,零 CDN 依赖 |
| 定位表述 | "选中文字,双击即译。不离开当前应用。" 大标题用行为描述,避免术语 |

---

## 当前能力速览

- 捕获:任意应用**双击 `Ctrl+C`** → 自动翻译 / 解释;单击只复制不触发。
- 捕获开关:托盘勾选 / 快捷键设置顶部开关,关闭后复制与唤起都不弹窗(托盘图标变灰、tooltip「捕获已暂停」)。
- 复制触发键:默认 `Ctrl+C` 双击触发,可改自定义组合键(单击触发)。
